...
module Numeric.Sundials.Solver
  ( CVMethod (..),
    ARKMethod (..),
    IDAMethod (..),
    Solver (..),
...
data Solver k where
  CVode :: Solver CVode
  ARKode :: Solver ARKode
  IDA :: Solver IDA

class SingSolver k where whichSolver :: Solver k

instance SingSolver CVode where whichSolver = CVode

instance SingSolver ARKode where whichSolver = ARKode

instance SingSolver IDA where whichSolver = IDA

type KnownFlags k =
  ( IsLabel "TOO_CLOSE" (Flag k),
    IsLabel "SUCCESS" (Flag k),
    IsLabel "ROOT_RETURN" (Flag k)
  )

type IsSolver k = (KnownFlags k, SingSolver k, Sundials k)
...
solveC ::
  ...
  Solver solver ->
  CConsts ->
  CVars (VS.MVector VSM.RealWorld) ->
  LogEnv ->
  IO (CInt, SundialsDiagnostics)
solveC solver CConsts {..} CVars {..} log_env =
  let report_error_new_api = wrapErrorNewApi (reportErrorWithKatip (getInt @solver #TOO_CLOSE) log_env)
   ...
   in do
        ...
        bracket (mkReport report_error_new_api) freeHaskellFunPtr $ \c_report_error -> do
          withSUNContext $ \sunctx -> do
            ...
            withNVector_Serial c_dim sunctx 6896 $ \y -> withIDAVectors solver c_dim sunctx $ \yp ids -> do
              -- /* Specify initial condition */
              VS.imapM_ (\i v -> cNV_Ith_S y i v) c_init_cond

              () <- case solver of
                IDA -> VS.imapM_ (\i v -> cNV_Ith_S yp i v) c_init_differentials
                CVode -> pure ()
                ARKode -> pure ()

              let go :: SolverObject solver -> IO (CInt, SundialsDiagnostics)
                  go mem = handleTermination @solver #SUCCESS (getDiagnostics mem implicit c_n_event_specs) $ do
                    let getDiagnosticsCallback loopState = getDiagnostics mem implicit c_n_event_specs loopState
                    () <- case solver of
                      CVode -> cCVodeInit mem c_rhs t0 y >>= check 1960
                      ARKode -> pure ()
                      IDA -> cIDAInit mem c_ida_res t0 y yp >>= check 1234
                    ...
                    when (c_fixedstep > 0.0) $ do
                      case solver of
                        ARKode -> cARKodeSetFixedStep mem c_fixedstep >>= check 8888
                    ...
                    withNVector_Serial c_dim sunctx 6471 $ \tv -> do
                      ...
                      when (c_n_event_specs /= 0) $ do
                        () <- case solver of
                          CVode -> cCVodeRootInit mem c_n_event_specs c_event_fn >>= check 6290
                          ARKode -> cARKodeRootInit mem c_n_event_specs c_event_fn >>= check 6290
                          IDA -> cIDARootInit mem c_n_event_specs c_event_fn_ida >>= check 6290
                      ...
                      withLinearSolver $ do
                        ...
                        when (c_jac_set /= 0) $ do
                          case solver of
                            CVode -> cCVodeSetJacFn mem c_jac >>= check 3124
                            ARKode
                              | implicit -> cARKodeSetJacFn mem c_jac >>= check 3124
                              | otherwise -> pure ()
                            IDA -> cIDASetJacFn mem c_jac_ida >>= check 3124
                        ...
                        let loop next_time_event = do
                              ...
                              (t, flag) <- allocaInStateT $ \t_ptr -> do
                                flag :: Flag solver <- case solver of
                                  CVode -> liftIO $ cCVode mem next_stop_time y t_ptr CV_NORMAL
                                  ARKode -> liftIO $ cARKodeEvolve mem next_stop_time y t_ptr ARK_NORMAL
                                  IDA -> liftIO $ cIDASolve mem next_stop_time t_ptr y yp IDA_NORMAL
                              ...
                              when (root_based_event || time_based_event) $ do
                                ...
                                (record_events, stop_solver, do_reinit) <-
                                  if (n_events_triggered > 0 || time_based_event)
                                    then do
                                      ...
                                      -- TODO: the three alloca can be removed, the c_apply_event is actually a pure haskell function
                                      (stop_solver, record_events, err, do_reinit) <- allocaInStateT $ \(stop_solver_ptr :: Ptr CInt) -> allocaInStateT $ \(record_event_ptr :: Ptr CInt) -> allocaInStateT $ \(do_reinit_ptr :: Ptr CInt) -> do
                                        --       /* Update the state with the supplied function */
                                        err <- do
                                          err :: CInt <- case solver of
                                            CVode -> do
                                              liftIO $ VSM.unsafeWith c_root_info $ \c_root_info_ptr -> c_apply_event (fromIntegral n_events_triggered) c_root_info_ptr t (coerce y) (coerce y) (coerce yp) stop_solver_ptr record_event_ptr do_reinit_ptr (\x -> pure x)
                                            ARKode -> do
                                              liftIO $ VSM.unsafeWith c_root_info $ \c_root_info_ptr -> c_apply_event (fromIntegral n_events_triggered) c_root_info_ptr t (coerce y) (coerce y) (coerce yp) stop_solver_ptr record_event_ptr do_reinit_ptr (\x -> pure x)
                                            IDA -> do
                                              ...
                                              let -- this is called by the event updater
                                                  ...
                                                  updateOnEvent new_y = do
                                                    ...
                                                    state <- readIORef ref
                                                    state' <- execStateT (idaReInit c_n_event_specs implicit mem t y yp) state
                                              ...
                                              err <- liftIO $ VSM.unsafeWith c_root_info $ \c_root_info_ptr -> c_apply_event (fromIntegral n_events_triggered) c_root_info_ptr t (coerce y) (coerce y) (coerce yp) stop_solver_ptr record_event_ptr do_reinit_ptr updateOnEvent
                                              state' <- liftIO $ readIORef ref
                                              put state'
                                              pure err
                                          pure err
                                        stop_solver <- liftIO $ peek stop_solver_ptr
                                        record_event <- liftIO $ peek record_event_ptr
                                        do_reinit <- liftIO $ peek do_reinit_ptr
                                ...
                                when (do_reinit > 0) $ do
                                  ...
                                  case solver of
                                    IDA -> idaReInit c_n_event_specs implicit mem t y yp
                                    _ -> do
                                      ...
                                      () <- case solver of
                                        CVode -> liftIO $ cCVodeReInit mem t y >>= check 1576
                                        ARKode -> do
                                          if not implicit
                                            then do
              ...
              case solver of
                CVode -> withCVodeMem c_method sunctx 8396 go
                ARKode
                  | not implicit -> withARKStepCreate c_rhs nullFunPtr t0 y sunctx 8396 go
                  | otherwise -> withARKStepCreate nullFunPtr c_rhs t0 y sunctx 8396 go
                IDA -> withIDACreate sunctx 8396 go

getDiagnostics :: forall solver. (IsSolver solver) => SolverObject solver -> Bool -> CInt -> LoopState -> IO SundialsDiagnostics
getDiagnostics cvode_mem implicit c_n_event_specs loopState = do
  -- /* Get some final statistics on how the solve progressed */
  nst <- cvGet sundialsGetNumSteps cvode_mem

  (nst_a :: CLong) <- case whichSolver @solver of
    ARKode -> cvGet cARKodeGetNumStepAttempts cvode_mem
    -- This value is not available
    CVode -> pure 0
    IDA -> pure 0

  (nfe :: CLong, nfi :: CLong) <- case whichSolver @solver of
    CVode -> do
  ...
  (ncfn, nje, nfeLS) <-
    if implicit
      then do
        ...
        (nfeLS :: CLong) <- case whichSolver @solver of
          CVode -> cvGet cCVodeGetNumLinRhsEvals cvode_mem
          ARKode -> cvGet cARKodeGetNumLinRhsEvals cvode_mem
          IDA -> cvGet cIDAGetNumLinResEvals cvode_mem
...
-- | Create the vector required for IDA to store yp and indexes
--
-- Note that it will return nullPtrVector for other method
-- This is inherently unsafe, can be improved
withIDAVectors :: Solver k -> SunIndexType -> SUNContext -> (N_Vector -> N_Vector -> IO a) -> IO a
withIDAVectors solver c_dim sunctx go = do
  let nullVector = N_Vector nullPtr
  case solver of
    IDA -> do
      withNVector_Serial c_dim sunctx 6896 $ \yp -> do
        withNVector_Serial c_dim sunctx 6896 $ \ids -> do
          go yp ids
    CVode -> go nullVector nullVector
    ARKode -> go nullVector nullVector

idaReInit :: CInt -> Bool -> (SolverObject IDA) -> CDouble -> N_Vector -> N_Vector -> StateT LoopState IO ()
idaReInit c_n_event_specs implicit ida_mem t y yp = do
  -- Before reinit, we get the diagnostics and update the current diagnostics
  state <- get
  diagnostics <- liftIO $ getDiagnostics ida_mem implicit c_n_event_specs state
  put $ state {current_diagnostics = diagnostics}
...
allocaInStateT :: (Storable t) => (Ptr t -> StateT state IO b) -> StateT state IO b
allocaInStateT callback = do
  state <- get
  (res, state') <- liftIO $ alloca $ \ptr -> do
    runStateT (callback ptr) state
  put state'
  pure res
